...
if MethodType = "Edit" then
	...
	if rsStatus.EOF or rsStatus.BOF then
		...
		Go_Result "Someone is playing silly buggers with the URL...", 0
	else
...
Moderation = "No"
Select Case MethodType
Case "Topic","Edit","Reply","ReplyQuote","TopicQuote","Forum","EditForum"
...
if MethodType = "EditTopic" then
	...
	if Len(Request.Form("Subject")) > 100 then 
		Err_Msg = Err_Msg & "<li>The Subject can not be greater than 100 characters</li>"
	end if
	if txtMessage = " " then 
...
if MethodType = "Topic" then
	...
	if rs.BOF or rs.EOF or not(QuoteOk) or not (ChkQuoteOk(strPassword))then '##  Invalid Password
		...
		if Len(Request.Form("Subject")) > 100 then 
			Go_Result "The Subject can not be greater than 100 characters", 0
			Response.End
		end if
...
sub Go_Result(str_err_Msg, boolOk)
	...
	Select Case MethodType
	Case "Topic","TopicQuote","Reply","ReplyQuote","Edit","EditTopic"
	...
	if boolOk = 1 then 
		Dim strOkMessage : strOkMessage = ""
		Select Case MethodType
			Case "Edit"
				strOkMessage = strOkMessage & "Your Reply Was Changed Successfully."
			...
			Case "EditCategory"
				...
				if strSubscription > 0 or strModeration > 0 then
					strOkMessage = strOkMessage & "Category Information Changed Successfully."
				else
					strOkMessage = strOkMessage & "Category Name Changed Successfully."
				end if
				strReturnURL = chkString(Request.Form("refer"),"refer")
				strReturnTxt = "Back To Forum"
			Case "EditForum"
				strOkMessage = strOkMessage & "Forum Information Updated Successfully."
				strReturnURL = chkString(Request.Form("refer"),"refer")
				strReturnTxt = "Back To Forum"
			Case "EditTopic"
				strOkMessage = strOkMessage & "Topic Changed Successfully."
				strReturnURL = "topic.asp?TOPIC_ID=" & Topic_ID
				strReturnTxt = "Back to the topic"
			Case "EditURL"
				strOkMessage = strOkMessage & "URL Information Updated Successfully."
				strReturnURL = chkString(Request.Form("refer"),"refer")
				strReturnTxt = "Back To Forum"
			Case "Reply", "ReplyQuote", "TopicQuote"
				...
				if Moderation = "Yes" then
					strOkMessage = strOkMessage & "New Reply Posted. It will appear once approved by a moderator."
				else
					strOkMessage = strOkMessage & "New Reply Posted."
					DoPCount
					if ForumCountMPosts <> 0 then
						DoUCount MemberID
					end if
				end if
				strOkMessage = strOkMessage & "</p><p>Thank you for your contribution."
			...
			Case "Topic"
				' DEM --> If moderated post, the counts should not be updated until after approval
				if Moderation = "Yes" then
					strOkMessage = strOkMessage & "New Topic Posted. It will appear once approved by a moderator."
				else
					strOkMessage = strOkMessage & "New Topic Posted."
					DoTCount
					DoPCount
					if ForumCountMPosts <> 0 then
						DoUCount MemberID
				...
				DoULastPost MemberID
			Case "Forum"
				strOkMessage = strOkMessage & "New Forum Created.</p>"
				strOkMessage = strOkMessage & "<p>The new forum is ready for users to begin posting."
				strReturnURL = chkString(Request.Form("refer"),"refer")
				strReturnTxt = "Back To Forum"
			case "URL"
				strOkMessage = strOkMessage & "New URL Created.</p>"
				strReturnURL = chkString(Request.Form("refer"),"refer")
				strReturnTxt = "Back To Forum"
			case "Category"
				strOkMessage = strOkMessage & "New Category Created.</p>"
				strOkMessage = strOkMessage & "Remember to create at least one new forum in this category."
				strReturnURL = chkString(Request.Form("refer"),"refer")
				strReturnTxt = "Back To Forum"
			Case Else
		...
		End Select
		Call OkMessage(strOkMessage,strReturnURL,strReturnTxt)
	else 
		Call FailMessage(str_err_Msg,True)
	end if 
	WriteFooter
  	Response.End
end sub
