	...
	case "DeleteReply"
		...
		if mLev > 0 then  '## is Member
			if (chkForumModerator(Forum_ID, strDBNTFUserName) = "1") or (mLev = 1) or (mLev = 4) then '## is Allowed
				...
				Call PopOkMessage("Reply Deleted",True)
			else
				Call FailMessage("<li>No Permissions to Delete Reply</li>",True)
			end if
		else
			Call FailMessage("<li>No Permissions to Delete Reply</li>",True)
		end if
	case "DeleteTopic"
		...
		if mLev > 0 then  '## is Member
			if (chkForumModerator(Forum_ID, strDBNTFUserName) = "1") or (mLev = 1) or (mLev = 4) then
				...
				next
				Call PopOkMessage("Topic Deleted",True)
			else
				Call FailMessage("<li>No Permissions to Delete Topic</li>",True)
			end if
		else
			Call FailMessage("<li>No Permissions to Delete Topic</li>",True)
		end if 
		...
		if mLev > 0 then  '## is Member
			if mLev = 4 then
				...
				for i = 0 to ubound(delAr) 
					...
					Application.UnLock
				next
				Call PopOkMessage("Forum Deleted",True)
			else
				Call FailMessage("<li>No Permissions to Delete Forum</li>",True)
			end if
		else
			Call FailMessage("<li>No Permissions to Delete Forum</li>",True)
		end if 
				...
				for i = 0 to ubound(delAr) 
					...
					rs.Open strSql, my_Conn
						do until rs.EOF
							...
							my_Conn.Execute ("DELETE FROM " & strTablePrefix & "ALLOWED_USERGROUPS WHERE FORUM_ID = " & cLng(rs("FORUM_ID"))),,adCmdText + adExecuteNoRecords
							rs.movenext
						loop
					rs.close
					set rs = nothing
					...
					Application.UnLock
				next
				Call PopOkMessage("Category Deleted",True)
			else
				Call FailMessage("<li>No Permissions to Delete Category</li>",True)
			end if
		else
			Call FailMessage("<li>No Permissions to Delete Category</li>",True)
		end if 
		...
		if mLev > 0 then  '## is Member
			if mLev = 4 then
				...
				if intDeleted > 0 then
					Call PopOkMessage("Member Deleted",True)
				else
					Call PopOkMessage("No Members Deleted",True)
				end if
			else
				Call FailMessage("<li>No Permissions to Delete Member</li>",True)
			end if
		else
			Call FailMessage("<li>No Permissions to Delete Member</li>",True)
		end if 
	case else
		Response.Write	"<form action=""pop_delete.asp?mode="
		select case Mode_Type
			case "Member"
		...
		if strAuthType = "db" then
			Response.Write	"<tr>" & vbNewLine & _
							...
							"<td class=""formvalue""><input type=""text"" maxLength=""25"" name=""User"" value=""" & chkString(strDBNTUserName,"display") & """ style=""width:150px;""></td>" & vbNewLine & _
							"</tr>" & vbNewLine & _
							"<tr>" & vbNewLine & _
							"<td class=""formlabel"">Password:&nbsp;</td>" & vbNewLine & _
							"<td class=""formvalue""><input type=""Password"" maxLength=""25"" name=""Pass"" value="""" style=""width:150px;""></td>" & vbNewLine & _
							"</tr>" & vbNewLine
		else
			if strAuthType = "nt" then
				Response.Write	"<tr>" & vbNewLine & _
      							"<td class=""formlabel"">NT Account:&nbsp;</td>" & vbNewLine & _
								"<td class=""formvalue"">" & strDBNTUserName & "</td>" & vbNewLine & _
								"</tr>" & vbNewLine
			end if
		end if
		Response.Write	"<tr>" & vbNewLine & _
