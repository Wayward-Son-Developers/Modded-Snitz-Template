...
if strSubscription = 1 and strEmail = 1 and strDBNTUserName <> "" then
	Response.Write	"<table width=""100%"" cellspacing=""0"">" & vbNewline
...
ShowLastHere = (mLev > 0)
if strShowStatistics <> "1" then
	Response.Write	"<table class=""shortstats"" width=""100%"" cellspacing=""0"">" & vbNewline & _
					"<tr>" & vbNewline & _
					"<td>"
	if ShowLasthere then 
		Response.Write	"You Last Visited - " & ChkDate(Session(strCookieURL & "last_here_date"), " " ,true)
	else
		Response.Write	"&nbsp;"
	end if 
	Response.Write	"</td>" & vbNewline & _
					"<td align=""right"">There are " & Posts & " Posts in " & Topics & " Topics and " & Users & " Users&nbsp;&nbsp;</td>" & vbNewline & _
					"</tr>" & vbNewline & _
					"</table>" & vbNewline
end if
...
Response.Write	"</td>" & vbNewline & _
				"<td>Topics</td>" & vbNewline & _
				"<td>Posts</td>" & vbNewline & _
				"<td nowrap>Last Post</td>" & vbNewline
if (strShowModerators = "1") or (mlev = 4 or mlev = 3) then
	Response.Write	"<td nowrap>Moderator(s)</td>" & vbNewline
end if 
...
If recCategoryCount = "" then
	...
	Response.Write	""">No Categories/Forums Found</td>" & vbNewline & _
					"<td>&nbsp;</td>" & vbNewline & _
					"</tr>" & vbNewline
else
	intPostCount  = 0
	...
	for iCategory = 0 to recCategoryCount
		...
		if (recForumCount = "" or not bContainsForum) and (mLev = 4) then
			...
			if Cat_ID = "" then
				Response.Write "<a href=""default.asp?CAT_ID=" & CatID & """ title=""View only the Forums in this Category"">" & ChkString(CatName,"display") & "</a></td>" & vbNewline
			else
				Response.Write "" & ChkString(CatName,"display") & "</td>" & vbNewline
			end if
			...
			Response.Write	"</tr>" & vbNewline & _
							"<tr>" & vbNewline & _
							"<td colspan=""" & sGetColspan(7,6) & """>No Forums Found</td>" & vbNewline & _
							"</tr>" & vbNewline
		else
				...
				if CatID = allForumData(fCAT_ID, iForum) then '## Forum exists
					...
					if ChkDisplayForum(ForumPrivateForums,ForumFPasswordNew,ForumID,MemberID) then
						...
						if chkDisplayHeader then
							...
							Response.Write	">" & _
											"<div class=""forumlink""><a href="""
							if ForumFType = 0 then
								Response.Write	"forum.asp?FORUM_ID=" & ForumID
							else 
								Response.Write	ForumURL & """ target=""_blank"
							end if 
							Response.Write	""">" & chkString(ForumSubject,"display") & "</a></div><br />" & vbNewLine & _
											"<div class=""forumdesc"">" & formatStr(ForumDescription) & "</div></td>" & vbNewline
							if ForumFType = 0 then
								if IsNull(ForumTopics) then 
									Response.Write	"<td class=""counts"">0</td>" & vbNewline
								else
									Response.Write	"<td class=""counts"">" & ForumTopics & "</td>" & vbNewline
								end if 
								if IsNull(ForumCount) then 
									Response.Write	"<td class=""counts"">0</td>" & vbNewline
								else
									Response.Write	"<td class=""counts"">" & ForumCount & "</td>" & vbNewline
								end if 
								if IsNull(ForumMemberID) then
									strLastUser = "&nbsp;"
								else
									strLastUser = "<br />by:&nbsp;" & profileLink(chkString(ForumMemberName,"display"),ForumMemberID)
									if strJumpLastPost = "1" then strLastUser = strLastUser & "&nbsp;" & DoLastPostLink(true)
								end if
								Response.Write	"<td class=""counts""><span class=""lastpost"">" & ChkDate(ForumLastPost, "<br />" ,true) & _
												"</span><span class=""lastuser"">" & strLastUser & "</span></td>" & vbNewline
							else 
								'## Do Nothing 
							end if 
							if (strShowModerators = "1") or (mlev = 4 or mlev = 3) then 
								Response.Write	"<td>" & listForumModerators(ForumID) & "</td>" & vbNewline
							end if 
							Response.Write	"<td class=""options"">"
...
Response.Write	"</table>" & vbNewline & _
				...
				"<tr>" & vbNewline & _
				"<td>" & vbNewline & _
				"<table class=""legend"" width=""100%"">" & vbNewline & _
				"<tr>" & vbNewline & _
				"<td>" & vbNewline & _
				getCurrentIcon(strIconFolderNew,"New Posts","align=""absmiddle""") & " Contains new posts since last visit.<br />" & vbNewline & _
				getCurrentIcon(strIconFolder,"Old Posts","align=""absmiddle""") & " No new posts since the last visit.<br /></td>" & vbNewline & _
				"</tr>" & vbNewline & _
				"</table>" & vbNewline
...
sub PostingOptions() 
	if (mlev = 4) or (lcase(strNoCookies) = "1") then 
		if Session(strCookieURL & "Approval") = strAdminCode then Response.Write("<a href=""down.asp"">" & getCurrentIcon(strIconLock,"Shut Down the Forum","hspace=""0""") & "</a>")
...
sub WriteStatistics() 
	...
	Response.Write	"<tr class=""section"">" & vbNewline & _
					"<td colspan=""" & sGetColspan(7,6) & """>Statistics</td>" & vbNewline & _
					"</tr>" & vbNewline & _
					"<tr>" & vbNewline & _
					"<td rowspan=""" 
	intStatRowSpan = 2
	...
	if ShowLastHere then 
		Response.Write	"<td colspan=""" & sGetColspan(6,5) & """>" & _
						"You last visited on " & ChkDate(Session(strCookieURL & "last_here_date"), " " ,true) & "</td>" & vbNewline & _
						"</tr>" & vbNewline & _
						"<tr>" & vbNewLine
	end if
	if intPostCount > 0 then
		...
		if Member_Count = 1 and User_Count = 1 then
			...
			Response.Write	Member_Count & " of " & User_Count & " <a href=""members.asp"">Members</a> have "
		end if
		Response.Write	" made "
		...
		if (LastPostDate = "" or LastPostLink = "" or intPostCount = 0) then 
			...
			Response.Write	", with the last post on " & LastPostLink & LastPostDate & "</a>"
	...
	if NewMember_Name <> "" then 
		Response.Write	"<tr>" & vbNewline & _
						"<td colspan=""" & sGetColspan(6,5) & """>Please welcome our newest member: " & profileLink(NewMember_Name,NewMember_Id) & ".</td>" & vbNewline & _
						"</tr>" & vbNewline
	end if 
end sub 
